...
class DatasetGTA(Dataset):
    ...
    def __init__(self, mode, dataset_specs):
        ...
        self.cont_thre = 0.2
        self.load_idx = dataset_specs.get('load_idx',False)
        self.idx_ckpt = dataset_specs.get('idx_ckpt','./data/idxs')
        ...
        for i, seq in tqdm(enumerate(os.listdir(data_file))):
            ...
            # *Luca:idx of the scene w/out the extension .npz = seq[:-4]
        ...
        for sub in tqdm(self.pose.keys()):
            ...
            for i in idxs_frame:
                ...
                if self.load_idx:
                    if self.mode == 'train':
                        idxs = np.load(f"{self.idx_ckpt}/{self.mode}/idxs_{sub}_{room}_{i}.npy")
                    else:
                        idxs = np.load(f"{self.idx_ckpt}/eval/idxs_{sub}_{room}_{i}.npy")
