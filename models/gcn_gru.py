...
class GCN_POSE(nn.Module):
    def __init__(self, model_specs):
        ...
        # !Change back 
        # self.sequence_pos_encoder_end = PositionalEncoding(nh_rnn)
        # self.embed_timestep_end = TimestepEmbedder(nh_rnn, self.sequence_pos_encoder_end)


        # TODO: change this to a more general way
        self.end_goal = True
        if self.end_goal:
            if wcont:
                out_in_dim += cont_dim
                out_in_dim_debug += cont_dim*2
                # TODO: change back to out_in_dim+3*2
        ...
        # Decode Pose
        # self.out_decode = nn.Sequential(nn.Linear(nh_rnn,nh_rnn),
        #                     nn.BatchNorm1d(nh_rnn),
        #                     nn.Tanh(),
        #                     nn.Linear(nh_rnn,out_dim),
    ...
    def forward(self, x, cont, cont_mask, aux=None, horizon=60,
        ...
        if self.wcont:
            # TODO: change back
            # hx only with root node and not all of the body as originally
        ...
        # !Change back
        # timestep_embedding_end = self.embed_timestep_end(torch.LongTensor([60]*bs))
        # timestep_embedding_end = timestep_embedding_end.squeeze()
...
#             timestep_embedding = self.embed_timestep(torch.LongTensor([i]*bs))
#             timestep_embedding = timestep_embedding.squeeze()
