...
from s3_pose_eval import main as main_eval
...
def loss_function(joints, y, root_traj, root_joint_idx,
    ...
    # *Added curriculum loss
    # loss_all = loss_joints + loss_root * pow(10, -(epoch*0.5 - 1))
...
def main(gpu, cfg, args):
    ...
    # *add argument for pre-trained trajectory model
    traj_weights = torch.load('./results/traj_gru_gcn/0050.p')
    model_dict = model.state_dict()
    traj_weights_dict = traj_weights['model_dict']
    traj_dict = {k: v for k, v in traj_weights_dict.items() if k in model_dict}
    model_dict.update(traj_dict)
    model.load_state_dict(model_dict) 

    # *Freeze the weights of the trajectory model
    for name, para in model.named_parameters():
        if name in traj_dict.keys():
