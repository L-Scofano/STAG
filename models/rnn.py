...
class GRU_POSE(nn.Module):
    ...
    def forward(self, x, cont, cont_mask, aux=None, horizon=60,
        ...
        if aux is not None:
            ...
            hx = x
        hx = self.x_enc(hx) # [seq,bs,nh_rnn]
        hx = self.x_gru(hx)[1][0] # [bs,nh_rnn]

        # encode cont
        if self.wcont:
            cont_tmp = cont.clone()
            # cont_tmp[:,:,:56] = 0
            hcont = self.cont_enc(cont_tmp) # [bs, seq, nh_rnn]
            hcont = self.cont_gru(hcont.transpose(0,1))[1][0] # [bs,nh_rnn]
