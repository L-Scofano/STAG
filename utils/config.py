...
class Config:
    ...
    def __init__(self, cfg_id, test=False):
        ...
        # * train wandb
        self.wandb_train = cfg.get('wandb_train', dict())
        self.wandb_test = cfg.get('wandb_test', dict())
        self.train_eval = cfg.get('train_eval', dict())
        
        # * create dirs
        self.base_dir = '/tmp' if test else 'results'

        self.name_dir = self.wandb_train['name']
        self.cfg_dir = '%s/%s' % (self.base_dir, self.name_dir)
        ...
        shutil.copyfile(cfg_name, '%s/%s.yml' % (self.cfg_dir,self.name_dir))
