...
#         self.x_stsgcn_enc = STSGCN_layer(3, nh_rnn, (1,1), 1, time_dim=30, joints_dim=1)
...
#         # * Encode the past root joint
#         hx = self.x_stsgcn_enc(root).permute(0,1,3,2).reshape(b, -1, t) # [bs, t, c*v]
#         hx = self.x_time_enc(hx).squeeze(-1).reshape(b,-1,1) # [b,c,v]
#         # hx = self.x_node_enc(hx).squeeze(-1) # [b,c]
...
#         if self.wcont:
#             root_h = self.g_w1(torch.cat([root_his_dct,hx.squeeze(),hcont],dim=1)) # [bs, 180+128+128] -> [BS, 180]||[BS, t, 128]
#         else:
...
#         #     # y_last = [bs, 150]
#         #     hy = self.out_enc(ylast)  # Decode Pose [bs, 128]
